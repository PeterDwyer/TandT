...
public class MetroEssentialsDemoUI : MonoBehaviourGUI
{
	void Start()
	{
		...
		SettingsPane.registerSettingsCommand( "My Magical License", "This app uses the internet. It's 2013 so deal with it" );
	...
	void OnGUI()
	{
		...
		{
			Snap.registerForSnapChanges( isSnapped =>
			{
				Debug.Log( "snap event fired. isSnapped: " + isSnapped );
			}, true );
		}
		...
		{
			// image in the app bundle. Note that you must put the image in the appropriate location for this to work!
		...
		if( GUILayout.Button( "Update Live Tile (TileSquareImage) with Remote Image" ) )
		{
			...
			var images = new string[] { "http://cchronicle.com/wp-content/uploads/2010/06/Stuff-Story-7102832.png" };
			Tiles.updateTile( TileTemplateType.TileSquareImage, null, images );
		}
