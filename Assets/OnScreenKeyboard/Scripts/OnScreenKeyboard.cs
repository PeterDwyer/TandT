...
public class OnScreenKeyboard : MonoBehaviour {
    ...
    public Vector2 stdKeySize = new Vector2(64, 64);
    public Vector2 lgeKeySize = new Vector2(128, 64);
	...
	string keyBackspaceString = "Backspace"; // Backspace
	public string keyBackspace = "<--"; // Backspace
	public string keyBackspaceAlt = "<<"; // Backspace
	...
	string[] upperKeysNew = { "Q", "W", "E", "R", "T", "Y", "U", "I", "O", "P", "<<", "<row>", 
                                    "A", "S", "D", "F", "G", "H", "J", "K", "L", "Done", "<row>", 
                                    "Caps", "Z", "X", "C", "V", "B", "N", "M", "_" };

    string[] lowerKeysNew = { "q", "w", "e", "r", "t", "y", "u", "i", "o", "p", "<<", "<row>", 
                                    "a", "s", "d", "f", "g", "h", "j", "k", "l", "Done", "<row>", 
                                    "Caps", "z", "x", "c", "v", "b", "n", "m", "-" };
	
//	string[] upperKeysNew = { "Q", "W", "E", "R", "T", "Y", "U", "I", "O", "P", "<<", "<row>", 
//                                    "Caps", "A", "S", "D", "F", "G", "H", "J", "K", "L", "Done", "<row>", 
//                                    "Shift", "Z", "X", "C", "V", "B", "N", "M" };
//
//    string[] lowerKeysNew = { "q", "w", "e", "r", "t", "y", "u", "i", "o", "p", "<<", "<row>", 
//                                   "Caps", "a", "s", "d", "f", "g", "h", "j", "k", "l", "Done", "<row>", 
//                                    "Shift", "z", "x", "c", "v", "b", "n", "m" };
	...
	public float pressedKeyScale = 0.9f;

	public float keyScaleRetina = 3.5f;
	public float fontScaleRetina = 3.5f;
	
	public bool keyDownRepeatSound = false;
	
	public bool iPadRetina = false;
...
//	}
	...
	void Awake()
	{
		if( iPadRetina || isiPadRetina() ) {
			stdKeySize.x *= keyScaleRetina; 	
			stdKeySize.y *= keyScaleRetina; 	
			lgeKeySize.x *= keyScaleRetina;
			lgeKeySize.y *= keyScaleRetina;
			keyStyle.fontSize = (int) ((float) keyStyle.fontSize * fontScaleRetina);
			if(iPadRetina) debugText = "iPad Retina Test";
		}
        ...
        for (int i = 0; i < upperKeys.Length; i++) {
			...
			if (upperKeys[i] == keyBackspaceAlt) upperKeys[i] = keyBackspace;			
        ...
        if (screenRect.x == 0 && screenRect.y == 0 && screenRect.width == 0 && screenRect.height == 0)
        {
			...
			maxWidth += stdKeySize.x*0.5f;
			maxHeight += stdKeySize.y*0.5f;
				
            screenRect = new Rect(Screen.width/2 - maxWidth/2, Screen.height - maxHeight, maxWidth, maxHeight);
        }
		...
		// scale standard
		if(pressedKeyScale == 0.0f) {
			pressedKeyScale = 1.0f;	
		}
        pressedStyle.fontSize = (int) ((float) keyStyle.fontSize * pressedKeyScale);
...
//		else if (Input.GetButton(joyPressButton)) KeyHeld();
	...
	private string ConvertBackspace(string keyText)
	{
		if( keyText.Equals(keyBackspace) || keyText.Equals(keyBackspaceAlt)) {
			return keyBackspaceString;
		}
		return keyText;
	}
	
	private string FilterInputKeys(string keyText)
	{
	...
	private void KeyPressed()
	{
		...
		//print ("KeyPressed: " + keyPressed + ", " + shiftState);
		
		keyPressed = ConvertBackspace(keyPressed);
		keyPressed = FilterInputKeys(keyPressed);
		
		if(!keyPressed.Equals("")) {
			GameObject.Find ("Manager").SendMessage("AddLetter", keyPressed);
		}
...
//			KeyPressed();
		...
		if (keyRepeatTimer < 0)
		{
			...
			print ("KeyHeld: " + keyPressed);
				
			keyRepeatTimer += continuedRepeatDelay;	

			if (keyPressSource != null && keyDownRepeatSound) keyPressSource.Play();
		}
    ...
    void OnGUI() 
	{	
		if(debug) { //!debugText.Trim().Equals("")) {
			GUI.Label(new Rect(0,0,300,20), debugText);
		}
...
//		print ("GetKeyPressed: " + key);
	...
	public bool isiPadRetina () {
		// is iPad Retina
		if(Application.platform == RuntimePlatform.IPhonePlayer && 
			(Screen.width == 2048.0f || Screen.height == 1536.0f) ) {
			debugText = "isiPadRetina!";
			iPadRetina = false;
			return true;
		}
		return false;
