using UnityEngine;
using System.Collections;

public class GirlOrBoy : MonoBehaviour 
{
	public LayerMask Mask;
	public bool tt;
	public RaycastHit hit;
	public GameObject target;
	public GameObject BoyManager;
	public GameObject GirlManager;
	public GameObject Boy;
	public GameObject Girl;
	public GameObject BoyB;
	public GameObject GirlB;
	public GameObject Next;
	public Ray ray;
	public Touch theTouch1 ;
	public GameObject T1;
	
	void Start () 
	{
	
	}
	
	void Update () 
	{
		//if(Input.touchCount > 0)
		if(Input.GetButton("Fire1"))
		{
			tt = true;
			//theTouch1 = Input.GetTouch(0);
			//ray = Camera.main.ScreenPointToRay(theTouch1.position);
			ray = Camera.main.ScreenPointToRay (Input.mousePosition);
			if(Physics.Raycast(ray,out hit,1000f,Mask))
			{
				target = hit.collider.gameObject;
				
				if(target == GirlB)
				{
					target.transform.localScale = new Vector3(4.068f,4.714213f,4.564f);
				}
				if(target == BoyB)
				{
					target.transform.localScale = new Vector3(4.0014f,4.714213f,4.995f);
				}
			}
		}
		else if(tt)
		{
			if(target != null)
			{
				if(target == BoyB)
				{
					Boy.SetActive(true);
					BoyManager.SetActive(true);
					Next.SetActive(true);
					T1.SetActive(true);
					Camera.main.SendMessage("SetStory", 2);
					
				}
				
				if(target == GirlB)
				{
					Girl.SetActive(true);
					GirlManager.SetActive(true);
					Next.SetActive(true);
					T1.SetActive(true);
					Camera.main.SendMessage("SetStory", 2);
				}
				
			//	target.transform.localScale = Vector3(4.701441,5.015121,4.896067);
			target = null;
			gameObject.SetActive(false);
			}
		}
	}
}